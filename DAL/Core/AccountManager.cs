...
namespace DAL.Core
{
    public class AccountManager : IAccountManager
    {
        ...
        {
            _context = context;
            _context.CurrentUserId = httpAccessor.HttpContext?.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        ...
        {
            if (await _context.Posts.Where(o => o.UpdatedById == userId).AnyAsync())
                return false;
        ...
        public async Task<(bool Succeeded, string[] Errors)> EditRolePermissionsAsync(ApplicationRole role, IEnumerable<string> claims)
        {
            if (claims != null)
            {
                string[] invalidClaims = claims.Where(c => ApplicationPermissions.GetPermissionByValue(c) == null).ToArray();
        ...
        public Task<SignInResult> PasswordSignInAsync(string username, string password, bool rememberMe, bool lockoutOnFailure)
        {
            var result = _signInManager.PasswordSignInAsync(username, password, rememberMe, lockoutOnFailure);
            return result;
        ...
        public async Task<(bool Succeeded, string[] Errors)> AddUserToRolesAsync(ApplicationUser user, IEnumerable<string> roles)
        {
            var result = await _userManager.AddToRolesAsync(user, roles);
            return (result.Succeeded, result.Errors.Select(e => e.Description).ToArray());
        }
        public async Task<(bool Succeeded, string[] Errors)> DeleteUserRolesAsync(ApplicationUser user, string[] Roles)
        {
            var result = await _userManager.RemoveFromRolesAsync(user, Roles);
            return (result.Succeeded, result.Errors.Select(e => e.Description).ToArray());
        }

        public async Task<List<ApplicationPermission>> GetRolePermissionsAsync(ApplicationRole Role)
        {
            var roleClaims = (await _roleManager.GetClaimsAsync(Role)).Where(c => c.Type == ClaimConstants.Permission);
            return roleClaims.Select(a => ApplicationPermissions.GetPermissionByValue(a.Value)).ToList();
        }
    }
}
