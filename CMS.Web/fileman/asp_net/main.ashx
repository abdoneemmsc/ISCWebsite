...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    {
        if (HttpContext.Current.User.IsInRole(RoleNames.admin)||HttpContext.Current.User.IsInRole(RoleNames.contentManager))
        {
            return true;
        }
        else
        {
            _r.Write(GetErrorRes("ليس لديك الصلاحيات لعمل هذا الإجراء"));
            return false;
        }
    }
